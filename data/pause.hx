...
function create(e){
    ...
    for(num => z in [
        ...
        add(z).antialiasing = true;
        if(num == 0)
            FlxTween.tween(z, {alpha: 0}, 1, {type: FlxTween.PINGPONG});
    }

    for(a in 0...3) {
        menuItems.push(new FunkinText(0, FlxG.height/9 * (a + 3.5), 0, ["Resume", "Restart", "Leave"][a], 32, true).screenCenter(FlxAxes.X));
        add(menuItems[a]).antialiasing = Options.antialiasing;
    }
